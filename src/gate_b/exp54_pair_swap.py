"""exp54 — budget-neutral PAIR swaps (reallocation moves) — method v2 addition.

Move: (i: +1, j: -1) simultaneously — the total budget changes by n_i - n_j; only feasible
moves are tested. Explores reallocation configurations unreachable by single-block swaps.
Acceptance: split-half validated (both halves improve). <=3 rounds; 5k eval.

Starts (auto-picked, tolerant to missing files):
  4.5: power_refined/4.5_power.json ; power_refined2/4.5_xl.json
  5.5: power_refined/5.5_*.json      ; power_refined2/5.5_xl.json
  5.0: power_refined/5.0_*.json      ; power_refined2/5.0_xl.json
Output: outputs/GateB/power_refined2/{tag}_pair.json
"""
import json
import sys
import time
...
START_FILES = {
    ...
    "5.5": ["outputs/GateB/power_refined/5.5_power_refined_cond_full_55_results.json",
            "outputs/GateB/power_refined2/5.5_xl.json",
            "outputs/GateB/uniform_starts.json"],
...
def main():
    ...
    for path in START_FILES[tag]:
        ...
        for r in range(1, MAX_ROUNDS + 1):
            ...
            if best is None:
                print(f"  [{tag}/{Path(path).stem} r{r}] pair-swap fixed point", flush=True)
                break
            (i, j, cA, cB), score = best
            bits[i] += 1
            bits[j] -= 1
            uA, uB = cA, cB
            print(f"  [{tag}/{Path(path).stem} r{r}] pair {i}+1/{j}-1 min-gain={score:+.5f} "
                  f"avg={avg(bits):.4f} ({time.time()-t0:.0f}s)", flush=True)
        results[Path(path).stem] = {"bits": bits, "avg_bit": avg(bits), "uA": float(uA), "uB": float(uB),
                                    "u_full": float(uA + uB)}

    if not results:
        print(f"[{tag}] NO STARTS FOUND"); return
    ...
    print(f"[{tag}] PAIR FINAL 5k: AP={m['AP']:.4f} AP50={m['AP50']:.4f}", flush=True)
    ...
    (OUT / f"{tag}_pair.json").write_text(json.dumps(rec, indent=2))
    applier.restore()
    print(f"EXP54 DONE [{tag}] AP={m['AP']:.4f}", flush=True)
